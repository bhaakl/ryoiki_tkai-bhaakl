...
class BaseActiveRecord extends \yii\db\ActiveRecord implements GridModelActionsInterface
{
    ...
    public function disableShowErrors()
    {
    ...
    public static function findActive()
    {
        ...
        $query = static::find();
        if (static::$safeDelete) $query->andWhere([static::tableName().'.is_deleted' => 0]);
        return $query;
    }
     ...
     * @return array
     */
    public static function listAll($keyField = 'id', $valueField = null, $sortByValue = true, $filter = [], $distinct = false)
    {
        if($valueField == NULL) {
     ...
     * @return array
     */
    public static function groupedListAll($keyField, $valueField, $groupLink, $groupName, $linkTable = '', $sortByValue = true, $filter = [])
    {
        $prefix = '';
        if($linkTable) {
            $prefix = $linkTable.'_';
        ...
        $query = static::findActive()
            ->select([static::tableName().'.'.$keyField, static::tableName().'.'.$valueField, $prefix.$groupName => $linkTable.$groupName])
            ->joinWith($groupLink)
            ->asArray();
        if ($sortByValue) {
            $query->orderBy([$prefix.$groupName => SORT_ASC, static::tableName().'.'.$valueField => SORT_ASC]);
        }
        ...
        return ArrayHelper::map($query->all(), $keyField, $valueField, $prefix.$groupName);
    }
