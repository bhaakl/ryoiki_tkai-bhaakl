 ...
 */
class OrderWithInsulationSearch extends OrderSearch
{
    ...
    public function search($params, $recsOnPage = 20)
    {
        $subQuery = OrderWProductionStageLink::find()
            ...
            ->where([
                'production_stage.code' => ProductionStage::STAGE_INSULATION
            ]);

        $subQuery2 = OrderWProductionStageLink::find()
            ->select([
                'order_id',
                'GROUP_CONCAT(insulation.id SEPARATOR ",") as insulations'
            ])
            ->joinWith('productionStage')
            ->joinWith('insulations')
            ->where([
                'production_stage.code' => ProductionStage::STAGE_INSULATION
            ])
            ->groupBy('order_id');
        ...
        $query = self::find()
            ...
            ->leftJoin(['insulations' => $subQuery2], 'order.id = insulations.order_id')
