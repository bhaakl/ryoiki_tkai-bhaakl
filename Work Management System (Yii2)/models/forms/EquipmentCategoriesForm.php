<?php

namespace app\models\forms;

use app\models\EquipmentCategory;
use yii\base\Model;

class EquipmentCategoriesForm extends Model
{
    public $equipment_categories;
    private $modelClassItem = EquipmentCategory::class;

    /** @var EquipmentCategory[]  */
    private $items = [];

    public function rules()
    {
        return [
            ['equipment_categories', 'validateEquipmentCategories']
        ];
    }

    public function validateEquipmentCategories($attribute)
    {
        foreach($this->$attribute as $index => $fields) {
            $model = new $this->modelClassItem();
            $model->scenario = $model::SCENARIO_INSERT;
            $model->load($fields, '');
            $model->disableShowErrors();
            if (!$model->validate()) {
                foreach ($model->getErrors() as $name => $error) {
                    $key = $attribute . '[' . $index . '][' . $name . ']';
                    $this->addError($key, $error);
                }
            } else {
                $this->items[] = $model;
            }
        }
    }

    public function save()
    {
        foreach ($this->items as $item) {
            if (!$item->save()) {
                return false;
            }
        }

        return true;
    }
}