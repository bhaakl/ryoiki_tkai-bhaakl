...
/* @var $searchModel app\models\ProductionLocation */
...
<?= GridViewClean::widget([
    ...
    'sortableAction' => $searchModel->category_id ? 'production-locations/sort' : '',
    'showPageSize' => true,
    'createRowModel' => new ProductionLocation(),
    ...
    'columns' => [
        [
            ...
            'value' => function($model) use ($can_update) {
                return $can_update ? Html::dropDownList('category_id', $model->category_id ?? null,
                    ProductionLocationCategory::listAll('id', 'name'), ['class' => 'form-control form-select select2', 'prompt' => 'Выберите']) : $model->category->name ?? '';
            },
        ],
        [
            'attribute' => 'name',
