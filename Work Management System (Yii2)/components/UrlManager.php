...
class UrlManager extends \yii\web\UrlManager {
    ...
    public function init()
    {
        ...
        $this->processMenuAccess($this->menuData);
    }

    private function processMenuAccess(&$menuData) {
        foreach ($menuData as $index => $menuItem) {
            if(!key_exists('access', $menuItem) && key_exists('url', $menuItem)) {
                list($controller, $actionId) = \Yii::$app->createController($menuItem['url']);
                if(!$controller) continue;
                $accessString = implode('_', [$controller->module->id, $controller->id, $controller->createAction($actionId)->id]);
                $menuData[$index]['access'] = $accessString;
            }
            if(key_exists('items', $menuItem)) {
                $this->processMenuAccess($menuData[$index]['items']);
            }
        }
    }
