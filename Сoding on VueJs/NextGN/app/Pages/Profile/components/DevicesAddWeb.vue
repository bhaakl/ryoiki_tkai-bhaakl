<template>
   <div class="cabinet__inputs-grid">
      ...
      <div class="cabinet__inputs-item">
         <div
            class="input-container"
            :class="{ 'has-error': $v.newBrowser.$dirty ? $v.newBrowser.$error : false }"
         >
            <p>Браузер</p>
            <selectize 
               v-model="newBrowser"
               id="browser" name="browser" placeholder="Браузеры">
               <option v-for="browser in browserOptions" :key="browser" :value="browser">{{ browser }}</option>
            </selectize>
            <label
               v-if="$v.newBrowser.$dirty ? $v.newBrowser.$error : false"
                  class="error ui-input__validation"
                  for="browser"
               >
                  Выберите браузер
...
export default {
   ...
   data() {
      return {
         ...
         newBrowser: null,
         newOSVersion: null,
         browserOptions: ['Chrome', 'Safari', 'Firefox', 'Opera', 'Edge', 'IE','Yandex'],
         osVersions: [],
      };
   },
   methods: {
      async addDevice() {
         ...
         if (!this.newBrowser || !this.newOSVersion) {
            return;
         }
         if ( this.isModal ){
            ...
            try {
               await this.$inertia.post(
                  `/profile/devices/web/add-device/`,
                  {
                     brand: this.newBrand,
                     browser: this.newBrowser,
                     o_s_version_id: this.newOSVersion,
