  ...
  <div class="card">
    <div class="card-header d-flex flex-row justify-content-between">
      <p class="font-weight-bold">
        Вопрос {{ index }}<span> (с закрывающим ответом)</span>
      </p>
      <CButton
        v-if="!group_name"
        color="light"
    ...
    <div class="card-body">
      ...
      >
        <template v-for="(answer, answerKey) in $v.answers.$each.$iter">
          ...
          />
          <CInputCheckbox
            :label="`Остановить выполнение заданий`"
            name="stop_tasks"
            style="margin-bottom: 10px; margin-top: -8px; margin-left: 5px;"
            :checked.sync="answer.close.$model"
            @change="() => {
              if(answer.close.$model) {
                answer.nestedQuestion.$model = null
                answer.isParentAnswer.$model = false
              }
            }"
          />
          <CInputCheckbox
            :label="`Обязательное комментирование ответа`"
            :checked.sync="answer.isCommentRequired.$model"
...
export default {
  ...
  data() {
    return {
      ...
      question: '',
      answers: [
        { id: 1, value: '', close: false, isCommentRequired: false, error: false, isParentAnswer: false, nestedQuestion: null },
        { id: 2, value: '', close: false, isCommentRequired: false, error: false, isParentAnswer: false, nestedQuestion: null }
      ],
  ...
  methods: {
    ...
    addAnswer() {
      this.answers.push({ id: this.answers.length + 1, value: '', close: false, isCommentRequired: false, error: false, isParentAnswer: false, nestedQuestion: null })
    },
    getData() {
      return {
        index: this.index,
